...
3. Async worker:
   ...
   - Searches Notion with `PRODUCT_NOTION_TOKEN`, **only** within explicit page/database allowlists
...
If both Notion allowlists are empty, Product Bot refuses to search (fail closed).
...
| `PRODUCT_NOTION_ALLOWED_PAGE_IDS` | yes* | Comma-separated page IDs |
| `PRODUCT_NOTION_ALLOWED_DATABASE_IDS` | yes* | Comma-separated database IDs |
| `PRODUCT_SLACK_ALLOWED_CHANNEL_IDS` | no | Comma-separated channel IDs; empty = all |

\* At least one of the Notion allowlists must be non-empty.
...
5. Create a Product Notion integration; share intended pages/databases; set `PRODUCT_NOTION_TOKEN`.
6. Set explicit `PRODUCT_NOTION_ALLOWED_*` allowlists.
...
- No Notion writes
- Fail closed when Notion allowlists are unset
- Never log tokens or signing secrets

## Rollout checklist

1. Confirm `GET /integrations/slack/product/health` shows Product credentials configured and allowlists set.
2. Optional: set `PRODUCT_SLACK_ALLOWED_CHANNEL_IDS` to a test channel first.
3. Enable `PRODUCT_BOT_ENABLED=1`.
4. `@`-mention the Product bot with a question grounded in allowlisted Notion pages.
